...
exports.OnGuildCreateEvent = void 0;
...
const OnGuildCreateEvent = async () => {
    try {
        ...
        main_1.client.on('guildCreate', (guild) => {
            try {
            ...
            try {
                if ((message.content.toLowerCase().includes('discord.gg') ||
                    ...
                    const embed = new builders_1.EmbedBuilder()
                        ...
                        .addFields({ name: '**المشرف المسؤول:**', value: `<@${main_1.client.user?.id}>`, inline: true }, { name: '**السبب:**', value: `User sent a blacklisted word!\n`, inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '**ردة الفعل:**', value: `Message was deleted!`, inline: true }, { name: '**الكلمة الممنوعة:**', value: `||\`Could not get the blacklisted word!\`||`, inline: true })
                        .setFooter({
                        ...
                        .setTimestamp();
                    await main_1.client.channels.cache.get('1042906729211498618').send({
                        embeds: [
                            embed
                        ]
                    });
                    message.deletable ? message.delete() : await main_1.client.channels.cache.get('1042906729211498618').send({
                ...
                for (var i = 0; i < cمسبات.length; i++) {
                    if (message.content.includes(cمسبات[i]) && !message.member?.permissions.has('Administrator')) {
                        const embed = new builders_1.EmbedBuilder()
                            .setDescription(`**رسالة قد ارسلت بواسطة <@${message.author.id}> و حذفت ب <#${message.channel.id}>. 🗑️!**\n|| ${message.content} ||`)
                            .addFields({ name: '**المشرف المسؤول:**', value: `<@${main_1.client.user?.id}>`, inline: true }, { name: '**السبب:**', value: `User sent a blacklisted word!\n`, inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '**ردة الفعل:**', value: `User has got timeout for \`6d, 23h, 59m, 59s\`!`, inline: true }, { name: '**الكلمة الممنوعة:**', value: `||\`${cمسبات[i]}\`||`, inline: true })
                            .setFooter({
                            text: message.guild?.name.toString() || '',
                            iconURL: message.guild?.iconURL({ size: 256 })?.toString()
                        })
        ...
        main_1.client.on('messageDelete', (message) => {
                ...
                if (message.attachments.size > 0 && message.attachments !== null) {
                    if (message.attachments.every(attachIsImage)) {
                        message.attachments.forEach(async (attachment) => {
                            const embed2 = new builders_1.EmbedBuilder()
                                ...
                                .setTimestamp();
                            const channel = message.guild?.channels.cache.get('1042906729211498618');
                            await channel.send({
                                embeds: [
                                    embed2
                                ]
        ...
        main_1.client.on('channelCreate', async (channel) => {
            ...
            for (var i = 0; i < cمسبات.length; i++) {
                if (channel.name.toLowerCase().includes(cمسبات[i])) {
                    ...
                    if (channel !== null) {
                        try {
                            channel.deletable ? channel.delete() : await main_1.client.channels.cache.get('1042906729211498618').send({
        ...
        main_1.client.on('channelUpdate', async (channel) => {
            for (var i = 0; i < cمسبات.length; i++) {
                if (channel.name.toLowerCase().includes(cمسبات[i])) {
                    const embed = new builders_1.EmbedBuilder()
                        .setDescription(`روم جديد قد حذف للشك بأنها سب! بـ <#${channel.id}>\nمحتوى اسم الروم :\n||\`\`\`${channel.name}\`\`\`||`)
        ...
        const guild = main_1.client.guilds.cache.get('1042879508493127721');
        setInterval(async () => {
...
exports.OnGuildCreateEvent = OnGuildCreateEvent;
